...
#define MEMS_ACCEL_FSR                  (8.0)
#define MEMS_GYRO_FSR                   (2000.0)
#define MEMS_DEFAULT_LPF                (42)
#define MEMS_SAMPLE_RATE                (10)
#define MEMS_SAMPLE_PERIOD              (1000 / MEMS_SAMPLE_RATE)     /* unit: ms */

  /* NONE CONFIGURABLE */
#define MEMS_FULL_CONST                 (32767)
#define MEMS_GYRO_FATOR                 (MEMS_FULL_CONST / MEMS_GYRO_FSR)
#define MEMS_ACCEL_FACTOR               (MEMS_FULL_CONST / MEMS_ACCEL_FSR)
#define MEMS_TIMESTAMP_STEP             (1000 / MEMS_SAMPLE_RATE)
...
#define i2c_read(slave_addr, reg_addr, length, data)  HAL_I2C_Mem_Read(&hi2c1, (slave_addr), (reg_addr), I2C_MEMADD_SIZE_8BIT, (data), (length), HAL_MAX_DELAY)
#define i2c_write(slave_addr, reg_addr, length, data) HAL_I2C_Mem_Write(&hi2c1, (slave_addr), (reg_addr), I2C_MEMADD_SIZE_8BIT, (data), (length), HAL_MAX_DELAY)
#define delay_ms              osDelay
#define get_ms(x)             (*(x) = osKernelSysTick())

/* labs and abs is already defined by Keil toolchain */
#define fabs(x)               (((x)>0)?(x):-(x))
#define min(a,b)              ((a<b)?a:b)
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (mpu_set_gyro_fsr(MEMS_GYRO_FSR))
        return -1;
    if (mpu_set_accel_fsr(MEMS_ACCEL_FSR))
        return -1;
    if (mpu_set_lpf(MEMS_DEFAULT_LPF))
        return -1;
    if (mpu_set_sample_rate(MEMS_SAMPLE_RATE))
        return -1;
